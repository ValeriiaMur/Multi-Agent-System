import type { ChatResponse, Route, TraceStep } from "../types";

export const ROUTE_META: Record<Route, { label: string; sub: string; glyph: string }> = {
...
export function deriveTrace(resp: ChatResponse): TraceStep[] {
  const pct = Math.round((resp.confidence ?? 0) * 100);
  const steps: TraceStep[] = [{ tool: "router.classify", arg: "intent", res: `${resp.route} · ${pct}%` }];
  if (resp.reason) steps.push({ tool: "reason", res: resp.reason });
...
export function deriveLead(resp: ChatResponse): string {
  if (resp.route === "WORKOUT_GENERATE") {
    const m = resp.workout?.meta;
    if (!resp.workout || m?.empty) return resp.reply;
...
export function clarifyChips(text: string): string[] {
  const short = text.trim().split(/\s+/).length <= 2;
  if (short) return [`Explain ${text}`, `Build around ${text}`, `Log ${text}`];
  if (/adjust|change|tweak|my workout/i.test(text)) return ["Log what I did", "Build a new session"];
